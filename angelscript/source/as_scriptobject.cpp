...
asCScriptObject::~asCScriptObject()
{
	...
	for( asUINT nn = 0; nn < objType->properties.GetLength(); nn++ )
	{
		/// CLOVER EDIT: Reverse order
		asUINT n= objType->properties.GetLength() - nn - 1;
		asCObjectProperty *prop = objType->properties[n];
		if( prop->type.IsObject() )
		{
			// Destroy the object
